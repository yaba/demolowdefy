...
function trimString(obj, expr, options, trimOpts) {
    var val = core_1.computeValue(obj, expr, null, options);
    var s = val.input;
    if (util_1.isNil(s))
        return null;
    var codepoints = util_1.isNil(val.chars)
        ? WHITESPACE_CHARS
...
function regexSearch(obj, expr, options, reOpts) {
    ...
    if (regexOptions) {
        util_1.assert(regexOptions.indexOf("x") === -1, "extended capability option 'x' not supported");
        util_1.assert(regexOptions.indexOf("g") === -1, "global option 'g' not supported");
    }
    var input = val.input;
